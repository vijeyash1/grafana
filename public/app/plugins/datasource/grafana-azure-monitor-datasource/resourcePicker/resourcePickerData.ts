...
export default class ResourcePickerData extends DataSourceWithBackend<AzureMonitorQuery, AzureDataSourceJsonData> {
  ...
  async fetchNestedRowData(resourceGroupOrSubscription: ResourceRow) {
    return resourceGroupOrSubscription.type === ResourceRowType.Subscription
      ? await this.getResourceGroupsBySubscriptionId(resourceGroupOrSubscription.id)
      : await this.getResourcesForResourceGroup(resourceGroupOrSubscription.id);
  }
